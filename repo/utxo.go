...
func (s *UTXOWalletRepo) UpdateWalletBalance(ctx context.Context, walletID int, delta decimal.Decimal) error {
	...
	if !delta.IsZero() {
		_, err := s.db.Exec(ctx, `
		INSERT INTO utxo_wallet (tx_id, wallet_id, amount, created_at, updated_at, spent)
...
func (s *UTXOWalletRepo) FindUnspentGTETarget(ctx context.Context, walletID int, target decimal.Decimal) ([]model.UTXOWallet, decimal.Decimal, error) {
	query := `
		WITH RECURSIVE candidate AS (
			SELECT u.tx_id, u.wallet_id, u.amount, u.created_at, u.updated_at,
				u.amount::NUMERIC(20,12) AS acc
			FROM (
				SELECT tx_id, wallet_id, amount, created_at, updated_at
				FROM utxo_wallet
				WHERE wallet_id = $1
				AND spent = false
				ORDER BY tx_id
	...
	for rows.Next() {
		...
		if err := rows.Scan(&utxo.TxID, &utxo.WalletID, &utxo.Amount, &utxo.CreatedAt, &utxo.UpdatedAt); err != nil {
			return []model.UTXOWallet{}, decimal.Zero, err
		}
